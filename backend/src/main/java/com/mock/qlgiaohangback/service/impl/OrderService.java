...
// Order Service chung
    ...
    public List<OrderRespDTO> getAllByShipperId() {
        ...
        return this.orderRepository
                .findOrderEntityByCarrier_Id(shipper_id).stream().map(IOrderMapper.INSTANCE::toOrderRespDTO).collect(Collectors.toList());
    }

    public List<OrderRespDTO> getAllByShipperIdByPage(String pageIndex, String pageSize) {
        Long shipper_id = Long.parseLong("6");
        ...
        Page<OrderEntity> ordersPaging = this.orderRepository.findOrderEntityByCarrier_Id(shipper_id,PageRequest.of(pageIndexInt - 1, pageSizeInt));
        List<OrderEntity> orderEntities = ordersPaging.getContent();
        return orderEntities.stream().map(IOrderMapper.INSTANCE::toOrderRespDTO).collect(Collectors.toList());
    }
...
//            System.out.println("-------NumberFormatException assignCarrier------");
        ...
        int id = orderRepository.updateCarrierIdAndStatus(String.valueOf(Constans.OrderStatus.REQUEST_SHIPPING), carrierIdLong, orderId, new Date());
        return id;
    }
    ...
    public List<OrderRespDTO> getOrderByDateAndCarrierId(OrderByDateAndCarrierId order) {
        ...
        List<OrderRespDTO> listOrder = orderRepository
                .findOrderEntityByCarrier_Id(carrierId).stream().map(IOrderMapper.INSTANCE::toOrderRespDTO).collect(Collectors.toList());
    ...
    public List<OrderEntity> updateOrderIsPaid(OrderByDateAndListId order) {
        ...
        List<OrderEntity> listOrder = orderRepository.findOrderEntityByCarrier_Id(carrierId);
